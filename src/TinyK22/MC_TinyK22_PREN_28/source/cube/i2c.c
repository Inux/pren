...
tError i2cReadCmdData(uint8_t adr, uint8_t cmd, uint8_t *data, uint8_t length)
{
  ...
  masterXfer.slaveAddress = adr;
  masterXfer.direction = kI2C_Read;
  masterXfer.subaddress = (uint32_t)cmd;
  masterXfer.subaddressSize = 1;
  masterXfer.data = data;
  masterXfer.dataSize = length - 1;
  masterXfer.flags = kI2C_TransferDefaultFlag;

  I2C_MasterTransferBlocking(I2C_1_PERIPHERAL, &masterXfer);
...
/*
 * ----------------------------------------------------------------------------------
 * I2C Functions from C.Jost - Module MC
 * ----------------------------------------------------------------------------------
 */
...
tError i2cStart(uint8_t adr, bool read)
{
  ...
  while (I2Cx_S & I2C_S_BUSY_MASK)
    ; // wait until i2c bus is idle. Necessary if two transmissions follow one after the other
  ...
  while (!(I2Cx_S & I2C_S_IICIF_MASK))
    ;   // wait for the transfer to complete
  I2Cx_S |= I2C_S_IICIF_MASK;             // clear the interrupt flag

  if (I2Cx_S & I2C_S_RXAK_MASK)
...
void i2cInit(void)
{
  // _todo ML#10.01 configure clock gating for i2c bus
  SIM_SCGC4_REG(SIM) |= SIM_SCGC4_I2C0_MASK;
  //CLOCK_EnableClock(s_i2cClocks[I2C_GetInstance(I2C0)]);


  // _todo ML#10.02 configure MUX for SCL & SDA --> done in pin-muxin tool
...
//  PORTD_PCR2 = PORT_PCR_MUX(7) | PORT_PCR_ODE(1);
  ...
  I2C0->F = 0x1D;  // 60 MHz/(1*160) = 375kHz => MULT=0, ICR=1D

  // _todo ML#10.04 enable i2c bus
